...
class SetupViewController: UIViewController, MPMediaPickerControllerDelegate {
    ...
    @IBOutlet weak var waveFormEQ: IHWaveFormView!
    ...
    override func viewDidLoad() {
        ...
        waveFormEQ.delegate = self
        waveFormEQ.dataSource = self
    ...
    func nowPlaying() {
        ...
        /*
        if let item = musicPlayer.nowPlayingItem {
            let image = item.artwork?.image(at: CGSize(width: 500, height: 500))
    ...
    func playAction() {
        waveFormEQ.audioPlayerDidStartPlaying()
        musicPlayer.play()
    ...
    func pauseAction() {
        // waveFormEQ.audioPlayerDidPausePlaying()
        musicPlayer.pause()
    ...
    func getMusicWinner() {
        ...
        /*
        var winnerVotes: Int = 0
        let eventInFirebase = DataService.connect.REF_EVENTS.child(eventID)
        eventInFirebase.child("voting").observeSingleEvent(of: .value, with: { (snapshot) in
            if let snapshot = snapshot.children.allObjects as? [DataSnapshot] {
                for snap in snapshot {
                    let key = snap.key as String
                    if let value = snap.value as? Int {
                        if winnerVotes < value {
                            winnerVotes = value
                            self.musicWinnerID = key
                        }
                    }
                }
            }
    ...
    // AVplayer experimental
    // https://stackoverflow.com/questions/30541570/play-items-from-mpmediaitemcollection-in-avplayer-swift
    /*
    func avPlayerTest() {
        let item = musicPlaylist.first
        let itemLocation = item?.value(forProperty: MPMediaItemPropertyAssetURL) as! URL
        
        // let avPlayerItem = AVPlayerItem(url: itemLocation)
        avPlayer = AVPlayer(url: itemLocation)
        avPlayer.play()
    }
     */
...
extension SetupViewController: IHWaveFormViewDataSource {
    
    func urlToPlay() -> URL {
        //Getting the Path of the Audio Asset in this case this is bundled in to the main Bundle with the fileName
        var url : URL?
        let playSource = musicPlayer.nowPlayingItem?.assetURL
...
extension SetupViewController: IHWaveFormViewDelegate {
